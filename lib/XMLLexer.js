...
class XMLLexer extends Transform {
  constructor (options = {}) {
  ...
  isClosing (pos) {
    const { awaited } = this; const { length } = awaited; if (length === 0) return true;
  ...
  getPosition () {
    const { beforeBody, start } = this;

    return start === 0 ? beforeBody : beforeBody + BigInt(start);
  }

  checkMaxLength () {
    const { body, maxLength } = this; const size = body.length - this.start;

    if (size <= maxLength) return;

    const s = body.slice(0, size - 1); const dump = JSON.stringify([s]);
