...
class XMLReader extends Transform {
  constructor (options = {}) {
  ...
  async findFirst () {
    ...
    return new Promise((ok, fail) => {
      ...
      const THE_END = 'finish'; const nope = () => ok(null);
  ...
  publish (xmlNode, type = null) {
    ...
    const { map } = this; const value = map === null ? xmlNode : map(xmlNode);
  ...
  flush_text () {
    ...
    if (text.length !== 0) {
      const e = new XMLNode(text, this.entityResolver, SAXEvent.TYPES.CHARACTERS);
  ...
  _transform (chunk, encoding, callback) {
    const { length } = chunk; if (length !== 0) {
      let e = new XMLNode(chunk, this.entityResolver); const { type } = e; const { element } = this;
