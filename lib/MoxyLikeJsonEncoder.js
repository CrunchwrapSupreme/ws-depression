const SAXEvent = require('./SAXEvent.js'); const { CHARACTERS, END_ELEMENT } = SAXEvent.TYPES
...
const MoxyLikeJsonEncoder = function (options = {}) {
  ...
  const xform = ({ children, attributes }) => {
    ...
    if (attributes != null) {
      for (const [name, value] of attributes.entries()) { set(value, attributes.getLocalName(name), attributes.getNamespaceURI(name)) }
    }
