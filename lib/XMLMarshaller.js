...
import stringEscape from 'string-escape-map';
import NamespacePrefixesMap from './NamespacePrefixesMap.js';
...
class XMLMarshaller {
  ...
  appendStartTag (node, data, qName) {
    ...
    if (!(this.isNsDumped || this.dont_dump)) {
      this.buf += this.ns;
  ...
  appendAttributes (node, data) {
    const { localName, namespaceURI, attributes, children, targetNamespace } = node; const { name, type, ref } = attributes;
      ...
      case 'anyAttribute':
        ...
        if (null in data) {
          for (const o of Object.values(data.null)) {
            for (const [k, v] of Object.entries(o)) { this.buf += ' ' + k + '="' + XML_ATTR.escape('' + v) + '"'; }
          }
        }
  ...
  appendContent (node, data) {
    ...
    switch (localName) {
      ...
      case 'element':
        ...
        const v = data[name]; if (v == null) return this.appendNullElement(node);
